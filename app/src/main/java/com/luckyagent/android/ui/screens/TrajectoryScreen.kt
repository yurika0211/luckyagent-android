...
@Composable
fun TrajectoryScreen(state: AppUiState, onRefresh: () -> Unit) {
    Column(
        Modifier
            .fillMaxSize()
            .background(CloverBg)
        ...
        Text("Trajectory", style = MaterialTheme.typography.headlineSmall, color = CloverText)
        Text("对照 GUI Trajectory：当前会话轨迹 / 工具步骤。", color = CloverText2, style = MaterialTheme.typography.bodyMedium)
        Button(onClick = onRefresh, modifier = Modifier.padding(vertical = 12.dp)) { Text("Refresh") }
        Text(
            text = state.trajectoryJson ?: "尚未加载",
